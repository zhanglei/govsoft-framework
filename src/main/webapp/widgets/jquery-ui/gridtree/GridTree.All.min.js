...
function HashMap()
{var size=0;var entry=new Object();this.put=function(key,value)
...
{var values=new Array();for(var prop in entry)
{values.push(entry[prop]);}
return values;}
this.keys=function()
{var keys=new Array();for(var prop in entry)
{keys.push(prop);}
return keys;}
this.size=function()
{return size;}
this.clear=function()
{size=0;entry=new Object();}}
jQuery.noConflict();function GridTree(){var tc;var headColumns;var parentToChildMap;var childToFatherMap;var nodeMap;var parents;var firstLevelNodes;var firstLevelParentIds;var pagingInfo;var _isValid;var _idToNumMap;var _style;var _lastSelectRowId;var _serverPagingMode;var TEXTBOX;var SELECT;var CHECKBOX;var RADIOBOX;var HTML;var isIE;var elct;var _usHandler;var _usnoclick;var _usnomsover;var _usnomsout;}
GridTree.prototype.makeTable=function(){if(!elct){alert('出错:要渲染div不存在!');return false;}
if(!_isValid){return false;}
var tableTree=document.createElement("table");if(tc.height!=null){tableTree.style.height=tc.height;}
if(tc.width==null)
tableTree.style.width='100%';else{tableTree.style.width=tc.width;}
tableTree.id=''+tc.tableId;tableTree.className='tableRegion';if(typeof tc.data!='undefined'){GridTree.prototype._makeTable(tableTree,tc.data);}else{if(_serverPagingMode=='client'){jQuery.ajax({type:"POST",url:tc.dataUrl,async:true,data:['idColumn=',tc.idColumn,"&parentColumn=",tc.parentColumn,"&gtlimit=",tc.pageSize,"&pageingMode=client"].join(''),success:function(msg){if(msg!=null&&msg!=""){try{eval(" tc.allDataInfoWithPageInfo="+msg);var data=tc.allDataInfoWithPageInfo.data;if(typeof data=='undefined'){data=tc.allDataInfoWithPageInfo;}
var columnName=tc['idColumn'];if(typeof data[0][columnName]=='undefined'){_isValid=false;alert("配置的属性[idColumn]值有误,请检查!");return;}
var columnName=tc['parentColumn'];if(typeof data[0][columnName]=='undefined'){_isValid=false;alert("配置的属性[parentColumn]值有误,请检查!");return;}
GridTree.prototype._makeTable(tableTree,data);}catch(e){GridTree.prototype._makeTableWithNoData(tableTree);}}else{elct.innerHTML="没有数据.";}}});}else{jQuery.ajax({type:"POST",url:tc.dataUrl,async:true,data:['idColumn=',tc.idColumn,"&parentColumn=",tc.parentColumn,"&gtlimit=",tc.pageSize].join(''),success:function(msg){if(msg!=null&&msg!=""){try{eval(" tableDataArr="+msg);var data=tableDataArr.data;var columnName=tc['idColumn'];if(typeof data[0][columnName]=='undefined'){_isValid=false;alert("配置的属性[idColumn]值有误,请检查!");return;}
var columnName=tc['parentColumn'];if(typeof data[0][columnName]=='undefined'){_isValid=false;alert("配置的属性[parentColumn]值有误,请检查!");return;}
GridTree.prototype._makeTable(tableTree,msg);}catch(e){GridTree.prototype._makeTableWithNoData(tableTree);}}else{elct.innerHTML="没有数据.";}}});}}
if(tc.disabeld){this.setDisabled(true);}}
GridTree.prototype.seeChildren=function(nodeid)
{var ansArr=parentToChildMap.get(nodeid);return ansArr;}
GridTree.prototype.isParent=function(nid){if(findInArray(parents,nid)!=-1){return'1';}else
return'0';}
GridTree.prototype.expandAll=function(){if(document.all){jQuery('#'+tc.tableId+' tr[_open=false]').each(function(i){var nodeId=this.id.replace('_node','');if(this._open=='false')
jQuery('#img_'+nodeId)[0].fireEvent('onclick');});}else{jQuery('#'+tc.tableId+' tr[_open=false]').each(function(i){var nodeId=this.id.replace('_node','');var evt=document.createEvent("MouseEvents");evt.initEvent("click",true,true);_$('img_'+nodeId).dispatchEvent(evt);});}}
GridTree.prototype.closeAll=function(){if(document.all){jQuery('#'+tc.tableId+' tr[_open=true]').each(function(i){var nodeId=this.id.replace('_node','');if(this._open=='true')
jQuery('#img_'+nodeId)[0].fireEvent('onclick');});}else{jQuery('#'+tc.tableId+' tr[_open=true]').each(function(i){var nodeId=this.id.replace('_node','');var evt=document.createEvent("MouseEvents");evt.initEvent("click",true,true);_$('img_'+nodeId).dispatchEvent(evt);});}}
GridTree.prototype.setDisabled=function(val){if(val){jQuery('.tableRegion input').attr('disabled','true');jQuery('.tableRegion button').attr('disabled','true');jQuery('.tableRegion select').attr('disabled','true');}
else{if(isIE){jQuery('.tableRegion input').each(function(i){if(this.userSetDisabled!='disabled'){jQuery(this).removeAttr('disabled');}});jQuery('.tableRegion button').each(function(i){if(this.userSetDisabled!='disabled'){jQuery(this).removeAttr('disabled');}});jQuery('.tableRegion select').each(function(i){if(this.userSetDisabled!='disabled'){jQuery(this).removeAttr('disabled');}});}else{jQuery('.tableRegion input').each(function(i){if(this.usersetdisabled!='disabled'){jQuery(this).attr('disabled',false);}});jQuery('.tableRegion button').each(function(i){if(this.usersetdisabled!='disabled'){jQuery(this).attr('disabled',false);}});jQuery('.tableRegion select').each(function(i){if(this.usersetdisabled!='disabled'){jQuery(this).attr('disabled',false);}});jQuery('.tableRegion input[usersetdisabled=disabled]').each(function(i){this.disabled=true;});jQuery('.tableRegion button[usersetdisabled=disabled]').each(function(i){this.disabled=true;});jQuery('.tableRegion select[usersetdisabled=disabled]').each(function(i){this.disabled=true;});}}}
GridTree.prototype.getSelectedRow=function(){return _$(_lastSelectRowId);}
GridTree.prototype.getRowObjById=function(nid){return _$('_node'+nid);}
GridTree.prototype.getNodePath=function(nid){var allParents=[];while(1){var pId;pId=childToFatherMap.get(nid);if(pId!=null){allParents.push(pId.replace('_node',''));if(findInArray(parents,pId)!=-1){nid=pId;}else{break;}}else{break;}}
return allParents.reverse();}
GridTree.prototype.openChildrenTable=function(imgid,node,e)
{var _id=imgid.replace('img_','').replace('_node','');var img=_$('img_'+_id);img.src=tc.closeImg;img.onclick=function(ee){GridTree.prototype.closeChildrenTable(imgid,node,ee)};var _preid=_id;jQuery('tr[_node_parent=_node'+_id+']').each(function(i){if(this._node_isparent=='1')
{GridTree.prototype.openChildrenTable(this.id,this,e);}
if(isIE)
this.style.display='inline';else{this.style.display='';}});_$('_node'+_id)._open='true';jQuery('tr[_node_parent=_node'+_id+']').show();this._setColor(_$(tc.tableId));if(isIE)
stopBubble();else{stopBubble(e);}}
GridTree.prototype.closeChildrenTable=function(imgid,node,e)
{var _id=imgid.replace('img_','').replace('_node','');var img=_$('img_'+_id);img.src=tc.openImg;img.onclick=function(ee){GridTree.prototype.openChildrenTable(imgid,node,ee)};_$('_node'+_id)._open='false';jQuery('tr[_node_parent=_node'+_id+']').each(function(i){if(this._node_isparent=='1')
{GridTree.prototype.closeChildrenTable(this.id,this,e);}
this.style.display='none';});this._setColor(_$(tc.tableId));if(isIE)
stopBubble();else{stopBubble(e);}}
GridTree.prototype.select=function(nodeId)
{var obj=jQuery('input[name=_chks][value='+nodeId+']')[0];if(isIE){if(_notBindDisabled(obj))
{jQuery(obj).attr('checked','true');}
stopBubble();}else{if(obj.getAttribute('userSetDisabled')==null||obj.getAttribute('userSetDisabled')!='disabled'){jQuery(obj).attr('checked','true');}
...
document.onclick=function(){var mndiv=_$('_menuDiv');if(mndiv)
mndiv.style.visibility="hidden";}
function toArray(ary){var result=new Array(ary.length);for(var i=0;i<ary.length;i++){result[i]=ary[i]}
return result;}
Function.prototype.bind=function(){var args=toArray(arguments);var owner=args.shift();var _this=this;return function(owner){return _this.apply(owner,args.concat(toArray(arguments)));}}
function importcss(csspath){var scripts=document.getElementsByTagName("link");for(var i=0;i<scripts.length;i++){if(csspath==scripts[i].getAttribute("href")){return;}}
if(isIE)
document.createStyleSheet(csspath);else{var headElement=document.getElementsByTagName("head")[0];var tempStyleElement=document.createElement('link');tempStyleElement.setAttribute("rel","stylesheet");tempStyleElement.setAttribute("type","text/css");tempStyleElement.setAttribute("href",csspath);headElement.appendChild(tempStyleElement);}}
function jsonMapToJsHashMap(jsonMap){var mapObj=new HashMap();for(var obj in jsonMap){mapObj.put(obj,jsonMap[obj]);}
return mapObj;}
function removeArrayFromOtherArray(arr1,arr2){var tempArr=[];var bingo=[];var len1=arr2.length;for(var ii=0;ii<len1;ii++){bingo.push(findInArray(arr1,arr2[ii]));}
var len2=arr1.length;for(var ii=0;ii<len2;ii++){if(findInArray(bingo,ii)==-1){tempArr.push(arr1[ii]);}}
return tempArr;}
function findInArray(arr,obj){var ans=-1;var len=arr.length;for(var i=0;i<len;i++){if(arr[i]==obj){ans=i;return ans;}}
return-1;}
function _checkedAll(checkName,v)
{var objs=document.getElementsByName(checkName);var len=objs.length;if(isIE){for(var i=0;i<len;i++)
{if(objs[i].userSetDisabled==null||objs[i].userSetDisabled!='disabled')
objs[i].checked=v;}}else{for(var i=0;i<len;i++){if(objs[i].getAttribute('userSetDisabled')==null||objs[i].getAttribute('userSetDisabled')!='disabled')
objs[i].checked=v;}}}
function appendText(node,txt){var nd=document.createTextNode(txt);node.appendChild(nd);}
function rmCss(obj,cssName)
{if(obj){if(includeClassInObj(obj,cssName)){obj.className=obj.className.replace(cssName,'').replace(/\s{2,}/g,' ');}}}
function includeClassInObj(obj,cssName){if(!obj.className){return false;}
return obj.className.indexOf(cssName)!=-1;}
function adCss(obj,cssName)
{if(obj){if(includeClassInObj(obj,cssName)){return;}
if(typeof obj.className!='undefined'){obj.className=obj.className+' '+cssName;}else{obj.className=cssName;}}}
function _notBindDisabled(o){if(o.userSetDisabled==null||o.userSetDisabled!='disabled'){return 1;}
return 0;}
function disableDom(obj,val)
{if(obj)
obj.disabled=val;}
function disableDomByName(domName,val)
{var obj=document.getElementsByName(domName);var len=obj.length;if(len>0){if(isIE){for(var i=0;i<len;i++){if(_notBindDisabled(obj[i]))
obj[i].disabled=val;}}else{for(var i=0;i<len;i++){if(obj[i].getAttribute('userSetDisabled')==null||obj[i].getAttribute('userSetDisabled')!='disabled')
obj[i].disabled=val;}}}}
...
function stopBubble(e){if(!isIE){e.preventDefault();e.stopPropagation();}else{window.event.cancelBubble=true;}}
function getAllCheckValue(){var chks=document.getElementsByName('_chks');var ans='';var len=chks.length;for(i=0;i<len;i++)
{if(chks[i].checked)
{ans+=chks[i].value+',';}}
ans=ans.substring(0,ans.length-1);return ans;}
function createRadio(el,dis,sty,name,val,click,innertext,cssname,chk){var rd;if(isIE)
{rd=document.createElement(["<input type='radio' name='",name,"' style='",sty,"' value='",val,"' ",dis," class='",cssname,"' userSetDisabled='",dis,"' ",chk,">"].join(''));}else{rd=document.createElement("input");rd.setAttribute('type','radio');rd.disabled=dis;rd.checked=chk;rd.setAttribute('style',sty);rd.setAttribute('name',name);rd.setAttribute('userSetDisabled',dis);rd.setAttribute('value',val);}
if(click)
rd.onclick=click;el.appendChild(rd);if(innertext!='')
el.appendChild(document.createTextNode(innertext));}
function createCheckbox(el,dis,sty,name,val,click,innertext,cssname,chk){var rd;if(isIE)
{rd=document.createElement(["<input type='checkbox' name='",name,"' style='",sty,"' value='",val,"' ",dis," class='",cssname,"' userSetDisabled='",dis,"' ",chk,">"].join(''));}else{rd=document.createElement("input");rd.setAttribute('type','checkbox');rd.disabled=dis;rd.checked=chk;rd.setAttribute('style',sty);rd.setAttribute('name',name);rd.setAttribute('userSetDisabled',dis);rd.setAttribute('value',val);}
if(click)
rd.onclick=click;el.appendChild(rd);if(innertext!='')
el.appendChild(document.createTextNode(innertext));}
function createHidden(id,name,val){var hid;if(isIE){hid=document.createElement(["<input type='hidden' id='",id,"' name='",name,"' value='",val,"'>"].join(''));}else{hid=document.createElement("hidden");hid.setAttribute('id',id);hid.setAttribute('name',name);hid.setAttribute('value',val);}
return hid;}
function createImg(imgsrc)
{var node=document.createElement('img');node.setAttribute('src',imgsrc);return node;}
function _$(id){return document.getElementById(id);}
Function.prototype.attachAfter=function(closure,functionOwner){var _this=this;return function(){this.apply(functionOwner,arguments);closure();}}
function attachEvent(obj,eventName,handler){obj[eventName]=(obj[eventName]||function(){}).attachAfter(handler,obj);}